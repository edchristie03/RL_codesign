...
shape_name, vertex = 'NA', [] # "Equi Triangle", [(-30, -30), (30, -30), (0, 30)] # "Square", [(-30, -30), (30, -30), (30, 30), (-30, 30)] #

# CMA-ES Parameters
LAMBDA = 10  # Population size (offspring per generation)
SIGMA0 = np.array([30.0, 1])  # Initial standard deviation
...
def objective_function(design_vector, generation, id, experiment_id):
    ...
    print(f"Evaluating Gen{generation}_ID{id} with design: {list(design_vector)}")

    # Train the model
    train(id, design_vector, generation, experiment_id)
...
def cmaes_optimization(experiment_id):
    ...
    # Initialize CMA-ES
    es = cma.CMAEvolutionStrategy(INITIAL_MEAN, 1.0, {  # Use 1.0 as base sigma
        'popsize': LAMBDA,
        'bounds': [BOUNDS[:, 0].tolist(), BOUNDS[:, 1].tolist()],
        'maxfevals': total_evaluations,
        'seed': 42,
        'verbose': 1,
        'CMA_stds': SIGMA0.tolist()  # This sets individual sigmas for each dimension
    })
    ...
    while generation < MAX_GENERATIONS and not es.stop():
        ...
        # make solutions integers and print
