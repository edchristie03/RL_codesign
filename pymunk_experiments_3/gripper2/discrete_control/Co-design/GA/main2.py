...
shape_name, vertex =  'NA', [] #"Wide_rectangle", [(-200, -10), (200, -10), (200, 10), (-200, 10)]  #"Square", [(-30, -30), (30, -30), (30, 30), (-30, 30)] # "Equi Triangle", [(-30, -30), (30, -30), (0, 30)] #
...
def train(id, design_vector, generation, experiment_id):
    ...
    eval_env = DummyVecEnv([make_env(vertex, 0, design_vector)])  # Set to False for faster training
...
def create_individual():
    ...
    design = (
        random.uniform(*BOUNDS['length']),
        random.uniform(*BOUNDS['links']),

    )
    return tuple(int(round(x, 0)) for x in design)
...
def mutate(individual):
    ...
    for i, value in enumerate(individual):
        if random.random() < 0.3:  # 30% chance to mutate each gene
            ...
            mutated.append(int(round(new_value, 0)))
        else:
